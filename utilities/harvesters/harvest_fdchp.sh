...
if [ $# -ne 3 ]; then
    ...
    echo "     example: $0 ce01issm D00001 20150505.fdchp.log"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
IN="$RAW/$PLATFORM/$DEPLOY/dcl12/fdchp/$FILE"
OUT="$PARSED/$PLATFORM/$DEPLOY/fdchp/${FILE%.log}.mat"

# Parse the file
if [ -e $IN ]; then
    $PYTHON $BIN/parse_fdchp.py -i $IN -o $OUT
fi
