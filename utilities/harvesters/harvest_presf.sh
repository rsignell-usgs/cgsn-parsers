#!/bin/bash
#
# Read the raw PRESF data files from the Endurance Coastal Surface Moorings and
...
if [ $# -ne 3 ]; then
    ...
    echo "     example: $0 ce02shsm D00001 20150505.presf.log"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
IN="$RAW/$PLATFORM/$DEPLOY/dcl35/presf/$FILE"
OUT="$PARSED/$PLATFORM/$DEPLOY/presf/${FILE%.log}.mat"

# Parse the file
if [ -e $IN ]; then
    $PYTHON $BIN/parse_presf.py -i $IN -o $OUT
fi
