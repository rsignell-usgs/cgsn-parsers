...
% and load the data into a single structured array
data = load([parsed_dir, '/', files(1).name], '-mat', '-regexp', '^(?!dcl_date_time_string).');
vrs = fieldnames(data);
for i = 2:length(files)
    tmp = load([parsed_dir, '/', files(i).name], '-mat', '-regexp', '^(?!dcl_date_time_string).');
